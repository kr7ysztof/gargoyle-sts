...
class STSApi(tokenService: TokenService) {
      ...
      pathSingleSlash {
        parameter("Action") {
          case action if "AssumeRoleWithWebIdentity".equals(action) =>
            parameters('RoleArn, 'RoleSessionName, 'WebIdentityToken, "DurationSeconds".as[Int] ? 3600) {
              (roleArn, roleSessionName, webIdentityToken, durationSeconds) =>
